	...
	\section{Plugin mechanism}
		...
		\subsection{The core world editing chain}
			\begin{tikzpicture}[node distance = 2cm, auto]
				\node [block] (a) {Selection};
				\node [block below of=a] (b) {Block iterator};
				\path [line] (a) -- (b);
				\node [block below of=b] (c) {Block changer};
				\path [line] (b) -- (c);
				\node [block below of=c] (d) {Cassette};
				\path [line] (c) -- (d);
				\node [block below of=d] (e) {User history};
				\path [line] (d) -- (e);
				\node [block below of=f] (g) {Load synchronizer};
				\path [line] (f) -- (g);
			\end{tikzpicture}
